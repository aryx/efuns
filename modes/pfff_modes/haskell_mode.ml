...
(* Prelude *)
(*****************************************************************************)
(* Tree-sitter-based Haskell major mode.
 *
 * This module provides a major mode to edit Haskell files by using
 * the Haskell parser and highlighter in respectively Semgrep and Codemap.
 *
 * todo:
 *  - indentation?
 *)
...
let color_buffer buf =
  ...
  UTmp.with_temp_file ~contents:s ~suffix:"hs" (fun file ->
    Pfff_modes.colorize_and_set_outlines Parse_and_highlight.haskell buf file
  )
...
(* The mode *)
(*****************************************************************************)
let hooks = Store.create_abstr "haskell_mode_hook"

let mode = Ebuffer.new_major_mode "Haskell(Semgrep)" (Some (fun buf ->
  color_buffer buf;
