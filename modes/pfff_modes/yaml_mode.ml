...
(* Prelude *)
(*****************************************************************************)
(* Tree-sitter-based YAML major mode.
 *
 * This module provides a major mode to edit YAML files by using
 * the YAML parser and highlighter in respectively Semgrep and Codemap.
 *
 * todo:
 *  - indentation?
 *)
...
let color_buffer buf =
  ...
  UTmp.with_temp_file ~contents:s ~suffix:"yaml" (fun file ->
    Pfff_modes.colorize_and_set_outlines Parse_and_highlight.yaml buf file
  )

(*****************************************************************************)
(* The mode *)
(*****************************************************************************)
let hooks = Store.create_abstr "yaml_mode_hook"

let mode = Ebuffer.new_major_mode "YAML(Semgrep)" (Some (fun buf ->
  color_buffer buf;
