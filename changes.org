...
* 0.26 (Q1 2026) (bugfixes, configure, magit, Haskell/Zig/Yaml/Dockerfile/Markdown)
# start to use Claude Code

** major modes
- new magit mode, port of Emacs magit mode
  (thx Claude)
- support Haskell, Zig, Yaml, Docker via semgrep-based tree-sitter parsers
- support Markdown via recursive descent parser
  (thx Claude)

** bugfixes
- fix stale top_second_cursor after C-g cancel in isearch/query-replace
  (Minibuffer.kill now clears top_second_cursor)
 (thx Claude)
- fix array out of bounds in Frame.set_cursor when long wrapped lines
  push the cursor y-coordinate past the frame height
 (thx Claude)

** build infra
- new configure (mostly copied from osemgrep) script
  (thx Claude)

** test infra
- added tests/bugfixes/ with manual reproduction steps for bug reports
  (thx Claude)
- Testo-based automated test suite (headless editor tests)
  (thx Claude)
...
** internals
- switch to semgrep-pfff-libs and semgrep-pfff-langs submodules instead
  of relying on ~/.opam or external/ and fragile OPAM_DIR symlinks

* -----------------------------------------------------------

* 0.25 (Q4 2025) (bugfixes)

** bugfixes
- do not raise divide by zero exn on files having a single line without
  a trailing \n when moving the cursor right

* 0.26 (Q3 2025) (Dockerfile and GHA CI, OCaml5 and ocaml-light ready)

** testinfra
- Dockerfile and docker.yml GHA (GitHub Actions) CI working for
  OCaml 4.14.2 and 5.2.1
- Dockerfile.light working with ocaml-light (for principia)
...
* 0.26 (Q1 2022) (Mode:Jsonnet, generic highlighters)

** languages
- a new Jsonnet mode (based on ojsonnet parser in semgrep)
- added a generic highlighter using the generic AST in semgrep

* Maintenance mode in 2020/2021/2022/2023/2024 as I was working on semgrep

* -----------------------------------------------------------

* 0.25 (Q4 2019) (Mode:Python)

** languages
- a new Python mode based on the Python parser and highlighter in Pfff.

** support for the bento code checker (r2c)
especially useful for Python and Javascript code
update: removed as bento got deprecated at r2c

* 0.24 (Q3 2019) ()

* 0.23 (Q2 2019) (Mode:Javascript)

** languages
- a new Javascript mode based on the Javascript parser and highlighter in Pfff.

* 0.22 (Q1 2019) (Macros!, cairo2)
...
* 0.21 (Q3 2018) (Merlin, Mode:Noweb, Shell history)
...
* 0.20 (Q2 2015) (Mode:Org/Outline/..., minimap, pad's stuff)
...
* 0.19 (Q1 2015) (Fork, Mode:Dircolors/Buffer_menu/Shell, pfff-based highlighters)
