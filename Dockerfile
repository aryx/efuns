# Build efuns (and semgrep and codemap) with OCaml 4.14.2 via OPAM on Ubuntu.
# Similar to codegraph Dockerfile
...
# Install codemap libs for efuns (see list below after dune install)
WORKDIR /codemap
# Add efuns external libs (which are also external libs of codemap)
# alt: use opam-depext
...
RUN eval $(opam env) && dune install \
    ...
    visualization \
    graph_code database_code \
    highlight_code highlighters \
    parser_nw parser_html
RUN rm -rf /codemap
...
# Test
RUN eval $(opam env) && efuns --help && efuns_client --help
